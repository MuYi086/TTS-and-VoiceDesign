...
主 API、`8301` 的 dots HTTP 包装器、`8302` 的 LongCat HTTP 包装器、IndexTTS2 和 Qwen 子进程使用同一个 conda 环境启动。由于 IndexTTS2 需要
...
- `8300`、`8301`、`8302` 共享同一个 `GPU_LOCK_FILE`，因此 Qwen / MiMo / IndexTTS2 / dots.tts / LongCat 不会并发抢占显存。
