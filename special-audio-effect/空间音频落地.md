...
以下链路先把源音频折叠为稳定的 mono 基准，再创建三层：

- `dry`：中心干声，保证对白清晰和单声道兼容；
- `wide`：过滤后的 Haas 湿声，提供左右时间/相位差；
- `room`：短延迟和早期反射，提供轻微头外感。
...
ffmpeg -hide_banner -y \
  ...
  -filter_complex "
    [0:a]
      ...
      asplit=3[center][wide_in][room_in];
    [center]
    ...
    [wide_in]
      ...
      highpass=f=180,
      lowpass=f=9000,
      haas=left_delay=2.05:right_delay=2.12,
      volume=0.24[wide];
    [room_in]
      aecho=0.8:0.25:'23|47':'0.11|0.055',
      pan=stereo|c0=c0|c1=c0,
      adelay='0|11S',
      lowpass=f=7000,
      volume=0.08[room];
...
这个默认值是“长时舒适”档，不追求参考 MP3 那样全程很宽。实测该组合把 30 秒片段的
相关系数中位数从接近 1.000 降到约 0.916，Side/Mid 中位数约为 −13.5 dB，同时没有
负相关块。

如果试听后仍觉得太窄，可按以下顺序调整，每次只改一项：

1. `wide volume` 从 `0.24` 提高到 `0.32`；
2. 仍不足时提高到 `0.40`，不要直接超过 `0.48`；
3. `room volume` 最多从 `0.08` 提高到 `0.12`；
4. 齿音扩散明显时，把 `lowpass=f=9000` 降到 `7500`；
5. 人声发虚时，先降低 `wide`，不要靠压缩器把人声重新顶回来。

`wide=0.40～0.48` 更接近短视频/预告片的夸张宽度，不建议未经长时间审听直接用于整本
有声书。
