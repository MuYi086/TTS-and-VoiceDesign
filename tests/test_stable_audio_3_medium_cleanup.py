...
class StableAudio3MediumCleanupTests(unittest.TestCase):
    def test_legacy_api_and_worker_are_removed(self) -> None:
        self.assertFalse((REPOSITORY_DIR / "api/stable_audio_3_medium_api.py").exists())
        self.assertFalse((REPOSITORY_DIR / "api/stable_audio_3_medium_worker.py").exists())
    ...
    def test_start_script_uses_only_the_standalone_uv_service(self) -> None:
        ...
        self.assertNotIn('python "$API_DIR/stable_audio_3_medium_api.py"', script)
        self.assertNotIn('python "$API_DIR/stable_audio_3_medium_worker.py"', script)
