"""Compatibility helpers for the currently published MOSS VoiceGenerator processor."""
...
def is_moss_codec_path_ready(codec_path: str | Path) -> bool:
    """Return whether a local codec has valid v1 metadata and weights."""
...
def split_sizes_from_break_positions(total_size: int, break_positions: Any) -> list[int]:
    ...
    MOSS-VoiceGenerator currently passes positions such as ``[80]`` directly
    to ``torch.split``.  Those positions mean ``idx[:80]`` and ``idx[80:]``;
    they are not valid split sizes unless the second segment is also added.
    """
...
def validate_moss_codec_compatibility(processor: Any) -> None:
    """Reject a codec checkpoint that is incompatible with VoiceGenerator."""
...
def install_moss_decode_compatibility(processor: Any) -> None:
    """Patch the processor instance without modifying its cached source file."""
