#!/usr/bin/env python3
"""One-shot MOSS-VoiceGenerator voice-design worker."""
...
def decode_message(processor, outputs):
    # The processor bundled with MOSS-VoiceGenerator uses torch.split() with
    # break positions instead of split sizes.  Patch only this instance so
    # cached/vendor model files remain untouched.
...
def join_waveforms(waveforms: list[Any], sample_rate: int, pause_ms: int, torch) -> np.ndarray:
    ...
    for waveform in waveforms:
        audio = waveform.detach().float().cpu().numpy() if isinstance(waveform, torch.Tensor) else np.asarray(waveform)
        if audio.ndim == 2:
...
def synthesize(request: dict[str, Any], output_wav: Path) -> None:
    ...
    # Fail before AutoProcessor/AutoModel emits the opaque "model_type" error
    # for an empty or partially downloaded local codec directory.
