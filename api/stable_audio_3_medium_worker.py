...
The worker owns the heavyweight model lifecycle.  It requires the official
Medium CUDA runtime and Flash Attention 2, writes one WAV, explicitly clears
CUDA state, and exits.  Process exit is the final VRAM-release guarantee.
"""
...
def require_model_path(path: str) -> Path:
    ...
    if not model_path.is_dir():
        raise FileNotFoundError(f"Local Stable Audio 3 Medium model directory is missing: {model_path}")
...
def import_runtime(upstream_path: Path):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "dependencies in the stable_audio_3_medium Conda environment. "
            f"Missing import: {exc.name or exc}"
        ) from exc
    return StableAudioModel, load_diffusion_cond, sf, torch


def require_cuda_and_flash_attention(torch: Any) -> None:
    """Apply Stable Audio 3 Medium's official GPU/runtime requirements."""
    ...
    if capability[0] < 8:
        raise RuntimeError(
            "Stable Audio 3 Medium requires an Ampere-or-newer CUDA GPU for Flash Attention 2; "
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "this environment's Python, PyTorch, and CUDA versions."
        ) from exc
...
def main() -> None:
    ...
    require_cuda_and_flash_attention(torch)
    ...
    try:
        ...
        # Delete model/audio references before allocator cleanup.  The parent
        # runner also terminates this worker after reading the output WAV.
